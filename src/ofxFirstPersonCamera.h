...
public:
  ...
  void updateRotationMouse( bool _constrainToYAxis = false )
  {
    ...
    setOrientation( tmpRotX * getOrientationQuat() * tmpRotY ); 
  }
  ...
  void updatePosition(float x, float y, float z){
    ...
    ofVec3f frameSpeed(0,0,0); // Todo: should we integrate over time as well?
    if( x > 0)  { frameSpeed -= getZAxis() * movementMaxSpeed; }
    else   { frameSpeed += getZAxis() * movementMaxSpeed; }
    
    if( y > 0 )   { frameSpeed -= getXAxis() * movementMaxSpeed; }
    else   { frameSpeed += getXAxis() * movementMaxSpeed; }
    
    //cout << "getYAxis(): " << getYAxis();
    
    
    if( z > 0)     { frameSpeed += getYAxis() * movementMaxSpeed; }
    else  { frameSpeed -= getYAxis() * movementMaxSpeed; }
